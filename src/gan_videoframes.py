...
OUTPUT_DIM = 64*64*4 # Number of pixels in each iamge
ITERS_TO_OUTPUT_IMG = 200
INPUT_SHAPE = [4, 64, 64]
...
def GoodGenerator(n_samples, noise=None, dim=DIM, nonlinearity=tf.nn.relu):
    ...
    output = tf.nn.relu(output)
    output = lib.ops.conv2d.Conv2D('Generator.Output', 1*dim, 4, 3, output)
...
def GoodDiscriminator(inputs, dim=DIM):
    output = tf.reshape(inputs, [-1] + INPUT_SHAPE)
    output = lib.ops.conv2d.Conv2D('Discriminator.Input', 4, dim, 4, output, he_init=False)
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    all_real_data_conv = tf.placeholder(tf.int32, shape=[BATCH_SIZE] + INPUT_SHAPE)
    ...
    def generate_image(iteration):
        ...
        lib.save_images.save_images(samples.reshape((BATCH_SIZE, 4, 64, 64)), 'samples_{}.png'.format(iteration))


    # Dataset iterator
    train_gen, dev_gen = lib.video_frames.load(BATCH_SIZE, data_dir=DATA_DIR)
    ...
    lib.save_images.save_images(_x_r.reshape((BATCH_SIZE/N_GPUS, 4, 64, 64)), 'samples_groundtruth.png')
    ...
    for iteration in xrange(ITERS):
        ...
        # Comment the 
        if iteration % ITERS_TO_OUTPUT_IMG == ITERS_TO_OUTPUT_IMG - 1:
