...
#DATA_DIR = '/home/lilo/Downloads/4_channel_dataset.npy'
#DATA_DIR = '/home/cuda/Tesina/code/dataset_3channel.npy'
DATA_DIR = '/home/nantinori/tesina/dataset/coco_3channel.h5p'
if len(DATA_DIR) == 0:
...
INPUT_WIDTH = 64
INPUT_HEIGHT = 64
OUTPUT_DIM = INPUT_WIDTH*INPUT_HEIGHT*INPUT_CHANNELS # Number of pixels in the dataset
LAZY_INCEPTION_SCORE_FILE = './lazy_inception_score.h5'

IMAGES_OUTPUT_DIR = './samples/%s' % time.strftime("%Y%m%d_%H%M%S")
...
def save_chunk(dataset, batch, size):
    ...
    dataset.resize((dataset_size + size, INPUT_WIDTH, INPUT_HEIGHT, 3))
...
def OptimizedResBlockDisc1(inputs):
    conv_1        = functools.partial(lib.ops.conv2d.Conv2D, input_dim=INPUT_CHANNELS, output_dim=DIM_D)
    conv_2        = functools.partial(ConvMeanPool, input_dim=DIM_D, output_dim=DIM_D)
    conv_shortcut = MeanPoolConv
    shortcut = conv_shortcut('Discriminator.1.Shortcut', input_dim=INPUT_CHANNELS, output_dim=DIM_D, filter_size=1, he_init=False, biases=True, inputs=inputs)
...
def Generator(n_samples, labels, noise=None, for_inception=False):
    if noise is None:
    ...
    # Added Generator.4 for scale up to 64x64 (add one more for 128x128 etc..)
    output = ResidualBlock('Generator.4', DIM_G, DIM_G, 3, output, resample='up', labels=labels)
    output = Normalize('Generator.OutputN', output)
    output = nonlinearity(output)
    output = lib.ops.conv2d.Conv2D('Generator.Output', DIM_G, INPUT_CHANNELS, 3, output, he_init=False)
    output = tf.tanh(output)
    if for_inception:
        # Get only the rgb channels (throw away the rest)
        return tf.transpose(tf.transpose(output, (1, 0, 2, 3))[:3], (1, 2, 3, 0))

    return tf.reshape(output, [-1, OUTPUT_DIM])

def Discriminator(inputs, labels):
    output = tf.reshape(inputs, [-1, INPUT_CHANNELS, INPUT_WIDTH, INPUT_HEIGHT])
...
with tf.Session() as session:
    ...
    def generate_image(frame, true_dist):
        ...
        lib.save_images.save_images(samples.reshape((100, INPUT_CHANNELS, INPUT_WIDTH, INPUT_HEIGHT)), IMAGES_OUTPUT_DIR + '/samples_{}.png'.format(frame))

    # Function for calculating inception score
    fake_labels_100 = tf.cast(tf.random_uniform([100])*10, tf.int32)
    samples_100 = Generator(100, fake_labels_100, for_inception=True)
    ...
    def get_inception_score(n):
        ...
        with h5py.File(LAZY_INCEPTION_SCORE_FILE, 'w') as hf:
            inception_dataset = hf.create_dataset('inception_dataset', data=np.empty((0, INPUT_WIDTH, INPUT_HEIGHT, 3), dtype='float32'), maxshape=(None, INPUT_WIDTH, INPUT_HEIGHT, 3))
            for i in xrange(n/100):
                batch_sample = session.run(samples_100)
                # Hacemos el floor en lugar del astype int32 para dejarlo en float para calcular el inception
                batch_sample = np.floor(((batch_sample+1.)*(255.99/2))) 
                save_chunk(inception_dataset, batch_sample, len(batch_sample))
        # Liberamos memoria por las dudas...
        del batch_sample
        return lib.inception_score.get_inception_score_lazy(LAZY_INCEPTION_SCORE_FILE)

    train_gen, dev_gen = lib.cifar10_video.load(BATCH_SIZE, DATA_DIR)
    def inf_train_gen():
        while True:
    ...
    _x, _y = inf_train_gen().next()
    _x_r = session.run(real_data, feed_dict={all_real_data_int: _x[:BATCH_SIZE/N_GPUS],all_real_labels:_y})
    _x_r = ((_x_r+1.)*(255.99/2)).astype('int32')
    lib.save_images.save_images(_x_r.reshape((BATCH_SIZE/N_GPUS, INPUT_CHANNELS, 64, 64)), IMAGES_OUTPUT_DIR + '/samples_groundtruth.png')
    ...
    for iteration in xrange(ITERS):
        ...
        for i in xrange(N_CRITIC):
            ...
            # Reshape de los datos que vienen del dataset..
            #_data = _data.reshape(BATCH_SIZE, OUTPUT_DIM)
            if CONDITIONAL and ACGAN:
        ...
        lib.plot.plot(IMAGES_OUTPUT_DIR + '/cost', _disc_cost)
        if CONDITIONAL and ACGAN:
        ...
        lib.plot.plot(IMAGES_OUTPUT_DIR + '/time', time.time() - start_time)

        if iteration % INCEPTION_FREQUENCY == INCEPTION_FREQUENCY-1:
            inception_score = get_inception_score(50000)
            lib.plot.plot(IMAGES_OUTPUT_DIR + '/inception_50k', inception_score[0])
            lib.plot.plot(IMAGES_OUTPUT_DIR + '/inception_50k_std', inception_score[1])
        ...
        if iteration % 200 == 199:
            ...
            lib.plot.plot(IMAGES_OUTPUT_DIR + '/dev_cost', np.mean(dev_disc_costs))
